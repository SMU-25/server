...
public class HomecamController {
    ...
    @PostMapping("/{childId}/{parentId}")
    public CustomResponse<HomecamResponseDTO.HomecamDTO> createHomecamTest(
            @PathVariable("childId") Long parentId,
            @RequestBody HomecamRequestDTO.HomecamRegisterDTO homecamRegisterDTO, @PathVariable("childId") Long childId){
        HomecamResponseDTO.HomecamDTO homecam = homecamService.createHomecamTest(homecamRegisterDTO,parentId, childId);
        return CustomResponse.onSuccess(homecam);
    }
}
